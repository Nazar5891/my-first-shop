...
export default function App() {
  ...
  const handleCreateListing = async (newListingData: Omit<Listing, 'id' | 'createdAt' | 'viewsCount' | 'callsCount' | 'distanceMeters'>): Promise<boolean> => {
    ...
    const listingData = { ...newListingData, authorId: user.uid, authorName: user.displayName || user.email?.split('@')[0] || 'Користувач', createdAt, viewsCount: 1, callsCount: 0 };
    try {
      ...
      setSelectedListing({ ...listingData, id: created.id, distanceMeters: dist });
  ...
  const handleDeleteListing = async (id: string): Promise<boolean> => {
    ...
    if (!target || target.authorId !== user.uid) return false;
    ...
    <div className="h-full min-h-screen flex flex-col bg-[url('https://images.unsplash.com/photo-1506703719100-a0f3a48c0f86?q=80&w=2000&auto=format&fit=crop')] bg-cover bg-center bg-fixed text-slate-100 font-sans selection:bg-purple-600 selection:text-white relative">
      ...
      <div className="relative z-10 h-full min-h-screen flex flex-col">
        ...
        <main className="flex-1 relative overflow-hidden flex flex-col">
          ...
          {activeTab === 'more' && <div className="p-3 sm:p-4 max-w-3xl mx-auto w-full"><MoreTab myListings={myListings} allListings={processedListings} onDeleteListing={handleDeleteListing} onSelectCategoryAndSubcategory={(catId, sub) => { setSelectedCategory(catId); setSelectedSubcategory(sub || null); setActiveTab('search'); }} /></div>}
        </main>
        ...
        <CallModal listing={callingListing} onClose={() => setCallingListing(null)} />
        <RouteModal listing={routingListing} onClose={() => setRoutingListing(null)} onStartOnlineNavigation={(listing) => { setActiveNavigationListing(listing); setActiveTab('map'); setSelectedListing(listing); }} />
        <ReportModal listing={reportingListing} onClose={() => setReportingListing(null)} />
