...
export default function App() {
  ...
  return <div className="h-full min-h-screen flex flex-col bg-cover bg-center bg-fixed text-slate-100 font-sans selection:bg-purple-600 selection:text-white relative" style={{ backgroundImage: `url("${SITE_BACKGROUND}")` }}>
    ...
    <div className="relative z-10 h-full min-h-screen flex flex-col">
      ...
      <ListingDetailBottomSheet listing={detailListing} onClose={()=>setDetailListing(null)} onCall={setCallingListing} onRoute={handleStartOnlineNavigation} onReport={setReportingListing} onAddComment={handleAddComment} onDeleted={id=>{setSelectedListing(null);setDetailListing(null);setActiveNavigationListing(active=>active?.id===id?null:active);}}/>
