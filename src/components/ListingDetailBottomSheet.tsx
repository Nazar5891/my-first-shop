...
export const ListingDetailBottomSheet: React.FC<
  ListingDetailBottomSheetProps
> = ({
  listing,
  onClose,
  onCall,
  ...
  const urgencyInfo = listing.urgencyLevel
    ? URGENCY_LEVELS_MAP[listing.urgencyLevel]
    : null;

  const isRideshare =
    listing.category === 'rideshare' ||
    Boolean(listing.rideRole);
  ...
  const hasCoordinates =
    ...
    Number.isFinite(listing.coordinates[0]) &&
    Number.isFinite(listing.coordinates[1]);
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(
      isAdmin && !isOwner
        ? 'Видалити це оголошення як адміністратор назавжди?'
    ...
    try {
      ...
      alert(
        'Не вдалося видалити оголошення. Перевірте підключення та права доступу.'
      );
    } finally {
      setIsDeleting(false);
    }
            ...
            >
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-black text-rose-300 text-sm uppercase">
                    Термінова допомога
                  </span>
