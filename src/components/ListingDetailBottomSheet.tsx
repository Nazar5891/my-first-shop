  ...
  return (
    <div className="fixed inset-0 z-[9990] flex justify-end bg-slate-950/50 backdrop-blur-[2px] animate-fade-in">
      ...
      <div className="relative w-full sm:w-[420px] md:w-[460px] h-[100dvh] max-h-[100dvh] bg-slate-950/98 backdrop-blur-2xl text-slate-100 shadow-2xl border-l-2 border-purple-600/80 flex flex-col overflow-hidden z-[9991] animate-slide-in-right">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-4 sm:p-6 pb-[170px] space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap pr-20">
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-extrabold border bg-purple-950/80 text-purple-200 border-purple-800/50"><span>{cat.pinSymbol}</span><span>{cat.label}</span></span>
            {listing.subcategory && <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-slate-900 text-cyan-300 border border-cyan-800/50">{listing.subcategory}</span>}
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-slate-900/90 text-purple-200 border border-purple-800/40"><MapPin className="w-3.5 h-3.5 text-purple-400" />📍 {formatDistance(listing.distanceMeters)} від вас</span>
            {listing.verified && <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-purple-950/60 text-purple-300 border border-purple-800/50"><CheckCircle2 className="w-3.5 h-3.5" />Номер перевірено</span>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="bg-slate-900/80 p-3 rounded-2xl border border-purple-900/40"><div className="flex items-center gap-1 text-purple-400 text-[11px] font-bold uppercase"><Calendar className="w-3.5 h-3.5" />Коли</div><p className="font-extrabold text-sm">{listing.when}</p></div>
            <div className="bg-slate-900/80 p-3 rounded-2xl border border-purple-900/40"><div className="flex items-center gap-1 text-purple-400 text-[11px] font-bold uppercase"><Clock className="w-3.5 h-3.5" />Тривалість</div><p className="font-extrabold text-sm">{listing.duration}</p></div>
            <div className="bg-slate-900/80 p-3 rounded-2xl border border-purple-900/40 col-span-2"><div className="flex items-center gap-1 text-purple-400 text-[11px] font-bold uppercase"><MapPin className="w-3.5 h-3.5" />Локація</div><p className="font-extrabold text-sm">{listing.locationName || 'Локацію не вказано'}</p></div>
          </div>
          ...
          {phone && <button type="button" onClick={handleCall} className="w-full rounded-2xl border-2 border-purple-500/70 bg-gradient-to-r from-purple-950/90 via-slate-900 to-cyan-950/70 p-4 shadow-[0_0_22px_rgba(124,58,237,0.22)] text-left hover:border-cyan-400/80 active:scale-[0.99] transition-all"><div className="flex items-center justify-between gap-3"><div><div className="text-[11px] font-black uppercase tracking-wide text-cyan-300 mb-1">Подзвонити</div><div className="text-lg font-black text-white tracking-wide">{phone}</div></div><div className="w-12 h-12 rounded-xl bg-purple-700/80 border border-purple-400/60 flex items-center justify-center shadow-lg"><Phone className="w-6 h-6 text-white" /></div></div></button>}
        ...
        <div className="relative shrink-0 z-[10000] p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] bg-slate-950/98 backdrop-blur-xl border-t-2 border-purple-500/80 shadow-[0_-12px_35px_rgba(0,0,0,0.85)]">
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={handleCall} className="min-h-[52px] py-3.5 bg-gradient-to-r from-purple-700 to-violet-600 hover:from-purple-600 hover:to-violet-500 active:scale-[0.98] text-white font-black rounded-xl flex items-center justify-center gap-2 shadow-lg border border-purple-400/60"><Phone className="w-5 h-5" />Подзвонити</button>
            <button type="button" onClick={() => onReport(listing)} className="min-h-[52px] py-3.5 bg-slate-900/95 hover:bg-rose-950 active:bg-rose-900 text-rose-300 font-black rounded-xl border border-rose-700/60 flex items-center justify-center gap-2 shadow-lg"><Flag className="w-5 h-5" />Поскаржитися</button>
          </div>
        </div>
      </div>
    </div>
