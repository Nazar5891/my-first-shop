...
import { X, Phone, MapPin, Clock, Calendar, Siren, Navigation, Eye, CheckCircle2, Flag, Share2, Star, MessageSquare, Send, UserCheck, Car, Trash2 } from 'lucide-react';
...
export const ListingDetailBottomSheet: React.FC<ListingDetailBottomSheetProps> = ({ listing, onClose, onCall, onRoute, onReport, onAddComment, onDeleted }) => {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    if (ok) { setNewCommentText(''); setNewAuthorName(''); setSuccessMessage('Дякуємо! Ваш відгук збережено.'); setTimeout(() => setSuccessMessage(''), 3000); }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onClose();
    } catch (error) { console.error('Не вдалося видалити оголошення:', error); alert('Не вдалося видалити оголошення. Перевірте підключення та права доступу.'); }
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40 backdrop-blur-[2px] transition-opacity animate-fade-in">
      ...
      <div className="relative w-full sm:w-[420px] md:w-[460px] h-full bg-slate-950/98 backdrop-blur-2xl text-slate-100 shadow-2xl border-l-2 border-purple-600/80 overflow-hidden flex flex-col z-10 animate-slide-in-right">
        ...
        <div className="absolute top-3 right-3 flex items-center gap-1.5 z-20">
          {canDelete && <button onClick={handleDelete} disabled={isDeleting} className="w-9 h-9 rounded-full bg-rose-950/90 hover:bg-rose-700 text-rose-200 border border-rose-600/60 flex items-center justify-center disabled:opacity-50" title={isAdmin && !isOwner ? 'Адмін: видалити оголошення' : 'Видалити моє оголошення'}><Trash2 className="w-4 h-4" /></button>}
          <button onClick={() => { if (navigator.share) navigator.share({ title: listing.title, text: `${listing.title} — ${listing.pay} (Помічник Громада)`, url: window.location.href }).catch(() => {}); else navigator.clipboard.writeText(window.location.href).then(() => alert('Посилання скопійовано!')).catch(() => {}); }} className="w-9 h-9 rounded-full bg-slate-900/90 hover:bg-purple-950 text-purple-300 border border-purple-800/40 flex items-center justify-center" title="Поділитися"><Share2 className="w-4 h-4" /></button>
        ...
        <div className="overflow-y-auto p-4 sm:p-6 space-y-4">
          ...
          {hasCoordinates && <button type="button" onClick={() => onRoute(listing)} className="w-full py-3 bg-white hover:bg-slate-200 text-slate-950 font-black text-xs uppercase tracking-wide rounded-xl border border-white flex items-center justify-center gap-2"><Navigation className="w-4 h-4" />Прокласти маршрут</button>}
          ...
          <div className="grid grid-cols-2 gap-2 pt-1"><button onClick={() => onCall(listing)} className="py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold rounded-xl flex items-center justify-center gap-1.5"><Phone className="w-4 h-4" />Зателефонувати</button><button onClick={() => onReport(listing)} className="py-3 bg-slate-900 hover:bg-rose-950 text-rose-300 font-extrabold rounded-xl border border-rose-800/50 flex items-center justify-center gap-1.5"><Flag className="w-4 h-4" />Поскаржитися</button></div>
        </div>
      </div>
    </div>
  );
