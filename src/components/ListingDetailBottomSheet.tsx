...
export const ListingDetailBottomSheet: React.FC<ListingDetailBottomSheetProps> = ({ listing, onClose, onCall, onRoute, onReport, onAddComment, onDeleted }) => {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Не вдалося зберегти відгук:', error);
  ...
  const handleShare = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: listing.title, text: `${listing.title} — ${listing.pay} (Помічник Громада)`, url: window.location.href });
      } else {
        await navigator.clipboard.writeText(window.location.href);
        alert('Посилання скопійовано!');
      }
    } catch { /* користувач закрив поширення */ }
  };

  return (
    <div className="fixed inset-0 z-[100] flex justify-end bg-slate-950/50 backdrop-blur-[2px] animate-fade-in">
      <div className="flex-1 hidden sm:block cursor-pointer" onClick={onClose} />
      <div className="relative w-full sm:w-[420px] md:w-[460px] h-[100dvh] max-h-[100dvh] bg-slate-950/98 backdrop-blur-2xl text-slate-100 shadow-2xl border-l-2 border-purple-600/80 flex flex-col overflow-hidden z-10 animate-slide-in-right">
        <div className="w-12 h-1.5 bg-purple-900/60 rounded-full mx-auto my-2 shrink-0 sm:hidden" />

        <div className="absolute top-3 right-3 flex items-center gap-1.5 z-[120]">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain p-4 sm:p-6 pb-32 space-y-4">
          ...
          <div><h4 className="text-xs font-extrabold text-purple-400 uppercase mb-2">Опис</h4><p className="text-sm text-slate-200 leading-relaxed whitespace-pre-line bg-slate-900/60 p-3.5 rounded-2xl border border-purple-900/30">{listing.description}</p></div>

          {phone && <div className="rounded-2xl border-2 border-emerald-500/60 bg-emerald-950/30 p-4 shadow-lg"><div className="text-[11px] font-black uppercase tracking-wide text-emerald-300 mb-2">Контактний номер</div><a href={`tel:${cleanPhone}`} className="flex items-center justify-between gap-3 rounded-xl bg-slate-950/70 border border-emerald-700/50 px-4 py-3 active:scale-[0.99] transition-transform"><span className="text-lg font-black text-white tracking-wide">{phone}</span><Phone className="w-5 h-5 text-emerald-400 shrink-0" /></a></div>}
        ...
        <div className="shrink-0 z-[110] border-t-2 border-purple-600/70 bg-slate-950/98 backdrop-blur-2xl px-4 sm:px-6 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-12px_35px_rgba(0,0,0,0.55)]">
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={handleCall} className="min-h-12 py-3 px-3 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-black rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-950/40"><Phone className="w-5 h-5" /><span>Зателефонувати</span></button>
            <button type="button" onClick={() => onReport(listing)} className="min-h-12 py-3 px-3 bg-slate-900 hover:bg-rose-950 active:bg-rose-900 text-rose-300 font-extrabold rounded-xl border border-rose-800/60 flex items-center justify-center gap-2"><Flag className="w-4 h-4" /><span>Поскаржитися</span></button>
          </div>
          {phone ? <div className="text-center text-[11px] text-slate-400 mt-2">Номер: <a href={`tel:${cleanPhone}`} className="text-emerald-300 font-bold">{phone}</a></div> : <div className="text-center text-[10px] text-slate-500 mt-2">Номер телефону в оголошенні не вказано</div>}
        </div>
      </div>
    </div>
  );
