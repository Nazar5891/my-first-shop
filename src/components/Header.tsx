   ...
   <div className="relative overflow-hidden rounded-xl border border-yellow-500/30 bg-gradient-to-b from-slate-900 to-slate-950 px-3 py-1.5 shadow-lg">
    ...
    <div className="flex items-center justify-between gap-2">
      <div className="flex min-w-0 items-center gap-2">
       <button type="button" onClick={()=>setAccountOpen(true)} aria-label="Мій акаунт" className="relative flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full border border-yellow-400/70 bg-slate-800 shadow-lg">{profilePhoto?<img src={profilePhoto} alt="Фото профілю" className="h-full w-full object-cover"/>:<UserRound className="h-4 w-4 text-yellow-300"/>}<span className={`absolute bottom-0 right-0 h-2 w-2 rounded-full border border-slate-950 ${user?'bg-emerald-400':'bg-slate-500'}`}/></button>
       ...
       <div className="min-w-0">
        ...
        <div className="flex items-center gap-1 text-[8px] font-bold text-blue-300"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-yellow-400"/><span>{user?'Акаунт онлайн':'Онлайн'}</span><span className="text-slate-500">• {totalListingsCount} оголошень</span></div>
       </div>
      </div>
