...
export const NearMeView: React.FC<NearMeViewProps> = ({
  ...
  // Separate urgent listings to pin them at the top of "ЗАРАЗ ПОРУЧ"
    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="flex items-center justify-between px-2 py-1.5 bg-slate-900/80 rounded-2xl border border-purple-900/40">
        <span className="inline-flex items-center gap-1.5 text-purple-200 text-xs font-bold">
          ...
          <span>Рокитнівська громада</span>
        </span>
      ...
      {/* Range Slider for Radius restriction */}
      <DistanceRangeSlider
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1">
        ...
        >
          ...
          <span>🚨 Тільки термінові</span>
        </button>
      </div>

      {/* URGENT HELP NEEDED TOP HIGHLIGHT SECTION */}
        ...
        <div className="space-y-2.5">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-xs font-extrabold text-rose-400 uppercase tracking-widest flex items-center gap-1.5">
              ...
              <span>НЕГАЙНА ТЕРМІНОВА ДОПОМОГА ({urgentListings.length})</span>
            </h3>
            <span className="text-[11px] font-black text-rose-200 bg-rose-950/80 border border-rose-800/60 px-2 py-0.5 rounded-full">
              Пріоритет №1
            </span>
          </div>
          ...
          <div className="space-y-2.5">
            {urgentListings.map((listing) => (
              <ListingCard
                ...
                onCallClick={() => onCallListing(listing)}
                variant="full"
              />
            ))}
          </div>
      ...
      {/* REGULAR NEAR ME LISTINGS */}
      {!onlyUrgent && (
        <div className="space-y-2.5">
          <div className="flex items-center justify-between px-1 pt-2">
            <h3 className="text-xs font-extrabold text-purple-300/80 uppercase tracking-widest">
              УСІ ПРОПОЗИЦІЇ ПОРУЧ ({regularListings.length})
            </h3>
            <span className="text-xs font-bold text-purple-300/60">
              Сортування: {sortBy === 'distance' ? 'за відстанню' : sortBy === 'pay' ? 'за оплатою' : 'нові'}
            </span>
          </div>

          {regularListings.length === 0 ? (
            <div className="cosmic-glass-card rounded-3xl p-8 text-center space-y-3">
              <div className="w-12 h-12 bg-purple-950/60 text-purple-300 rounded-full flex items-center justify-center mx-auto border border-purple-800/40">
                📍
              </div>
              <p className="text-sm font-bold text-slate-200">Оголошень за цими фільтрами не знайдено</p>
              <button
                onClick={() => {
                  setSortBy('distance');
                  setOnlyUrgent(false);
