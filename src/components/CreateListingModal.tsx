...
const MAIN_CATEGORY_IDS: CategoryId[] = [
  'service',
  'rideshare',
  'sale',
  'buy',
      ...
      try {
        const tryCompress = (side: number, quality: number) => {
          ...
          canvas.toBlob(
            (blob) => {
              if (!blob) {
                reject(new Error('Не вдалося конвертувати фото у WebP.'));
                return;
              }
              ...
              reader.onload = () => {
                ...
                if (quality > 0.55) {
                  tryCompress(side, Math.max(0.55, quality - 0.07));
                  return;
                }

                if (side > 900) {
                  tryCompress(900, 0.78);
                  return;
                }

                if (side > 720) {
                  tryCompress(720, 0.75);
                  return;
                }

                if (side > 600) {
                  tryCompress(600, 0.72);
                  return;
                }
                ...
                reject(
                  new Error(
                    'Фото не вдалося стиснути до безпечного розміру. Спробуйте інше фото.'
                  )
                );
              };
              ...
              reader.onerror = () => {
                URL.revokeObjectURL(objectUrl);
                reject(new Error('Не вдалося прочитати стиснене фото.'));
              };
...
export const CreateListingModal: React.FC<CreateListingModalProps> = ({
  ...
  const [urgencyLevel, setUrgencyLevel] =
    useState<UrgencyLevel>('immediate');

  const [urgentType, setUrgentType] =
    useState<UrgentHelpType>('auto');
  ...
  const selectedCoordinates =
    coordinates ||
    (gpsEnabled && userCoordinates ? userCoordinates : null);
  ...
  useEffect(() => {
    ...
    setPhotoUrl('');
    setPhotoPreview('');
    setPhotoOriginalSize(0);
    setPhotoCompressedSize(0);
  }, [isOpen]);
  ...
  const handlePhoto = async (file?: File) => {
    ...
    if (!file.type.startsWith('image/')) {
      setErrorMessage('Оберіть саме фото.');
      return;
    }

    if (file.size > MAX_SOURCE_PHOTO_BYTES) {
      setErrorMessage('Фото завелике. Максимум 10 МБ.');
      return;
    }
    ...
    try {
      ...
      setPhotoCompressedSize(result.size);
    } catch (e) {
      setPhotoUrl('');
      setPhotoPreview('');
      setPhotoOriginalSize(0);
  ...
  const determineAutomatically = () => {
    if (!navigator.geolocation) {
      setErrorMessage(
        'Цей браузер не підтримує визначення місцезнаходження.'
      );
      return;
    }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!canPublish) {
      setErrorMessage(
        '📍 Спочатку визначте місце автоматично або виберіть точку на карті.'
      );
      return;
    }

    if (!title.trim()) {
      setErrorMessage('Вкажіть заголовок оголошення.');
      return;
    }
    ...
    try {
      ...
      if (!ok) {
        throw new Error(
          'Не вдалося зберегти оголошення. Перевірте Firebase.'
        );
      }
      ...
      setErrorMessage(
        e instanceof Error
          ? e.message
          : 'Не вдалося опублікувати оголошення. Перевірте інтернет.'
      );
    }
  };
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md overflow-hidden"
        style={{
      ...
      >
        <div className="h-full flex items-center justify-center p-2">
          <div className="relative w-full max-w-xl h-[calc(100dvh-16px)] bg-slate-950/95 text-slate-100 rounded-3xl shadow-2xl border border-purple-900/50 flex flex-col overflow-hidden">
            ...
            <div className="shrink-0 p-4 border-b border-purple-900/40 flex items-center justify-between bg-slate-950">
              <div>
                ...
                <p className="text-[11px] text-purple-300/70">
                  Оберіть категорію та підкатегорію
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-9 h-9 rounded-full bg-slate-900 text-purple-300 border border-purple-800/40 flex items-center justify-center"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {/* FIXED PUBLISH BAR */}
            {!success && (
              <div className="shrink-0 z-40 p-3 sm:p-4 bg-slate-950 border-t border-purple-900/60 shadow-[0_-10px_30px_rgba(0,0,0,0.45)]">
                <button
                  disabled={
                    busy ||
                    processingPhoto ||
