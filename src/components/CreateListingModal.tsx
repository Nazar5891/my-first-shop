...
import { X, Phone, Siren, Camera, CheckCircle2, AlertCircle, Send, MapPin } from 'lucide-react';
...
export const CreateListingModal: React.FC<CreateListingModalProps> = ({ isOpen, onClose, onSubmit, userCoordinates = COMMUNITY_CENTER }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMessage('');
    if (!hasRealGps) {
      return setErrorMessage('📍 Не вдалося визначити ваше місцезнаходження. Увімкніть геолокацію та спробуйте ще раз. Ми більше не будемо автоматично ставити оголошення в центрі Рокитного.');
    }
      ...
      <div className="relative w-full max-w-xl bg-slate-950/95 text-slate-100 rounded-3xl shadow-2xl border border-purple-900/50 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4">
          {success ? <div className="py-8 text-center space-y-4"><CheckCircle2 className="w-16 h-16 mx-auto text-emerald-400" /><h3 className="text-xl font-black">Оголошення опубліковано!</h3><p className="text-sm text-purple-200/80">Оголошення збережено у Firebase.</p><button onClick={onClose} className="px-8 py-3 bg-purple-600 rounded-2xl font-extrabold">Готово</button></div> : <form onSubmit={handleSubmit} className="space-y-4">
            <div className={`p-3 rounded-2xl border text-xs font-bold flex gap-2 ${hasRealGps ? 'bg-emerald-950/50 text-emerald-200 border-emerald-800/60' : 'bg-amber-950/60 text-amber-200 border-amber-800/60'}`}><MapPin className="w-4 h-4 shrink-0" /><span>{hasRealGps ? 'Ваше реальне місцезнаходження визначено. Оголошення буде розміщено за вашими координатами.' : 'Геолокація вимкнена або недоступна. Оголошення НЕ буде автоматично розміщене в центрі Рокитного.'}</span></div>
            ...
            <button disabled={busy || !hasRealGps} type="submit" className={`w-full py-3.5 rounded-2xl font-black text-white flex items-center justify-center gap-2 disabled:opacity-50 ${isUrgent ? 'bg-rose-600' : 'bg-gradient-to-r from-purple-600 to-indigo-600'}`}><Send className="w-4 h-4" />{busy ? 'Публікація…' : hasRealGps ? 'Опублікувати оголошення' : 'Увімкніть геолокацію для публікації'}</button>
          </form>}
        </div>
      </div>
    </div>
