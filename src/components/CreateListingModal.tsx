...
import { X, MapPin, Phone, Siren, Camera, CheckCircle2, AlertCircle, Send } from 'lucide-react';
...
export const CreateListingModal: React.FC<CreateListingModalProps> = ({ isOpen, onClose, onSubmit, userCoordinates = COMMUNITY_CENTER }) => {
  ...
  const [coords] = useState<[number, number]>(userCoordinates);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-xl bg-slate-950/95 text-slate-100 rounded-3xl shadow-2xl border border-purple-900/50 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        <div className="p-4 sm:p-5 border-b border-purple-900/40 flex items-center justify-between">
          ...
          <button onClick={onClose} className="w-9 h-9 rounded-full bg-slate-900 text-purple-300 border border-purple-800/40 flex items-center justify-center"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4">
          {success ? (
            <div className="py-8 text-center space-y-4">
              ...
              <p className="text-sm text-purple-200/80">Жодних SMS-кодів або кодів видалення більше немає.</p>
              <button onClick={onClose} className="px-8 py-3 bg-purple-600 rounded-2xl font-extrabold">Готово</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {(Object.keys(CATEGORIES) as CategoryId[]).filter(k => k !== 'sale').map(k => (
                    <button key={k} type="button" onClick={() => { setCategory(k); setSubcategory(''); }} className={`p-2.5 rounded-2xl text-left border ${category === k ? 'bg-purple-600 text-white border-purple-400' : 'bg-slate-900 text-purple-200 border-purple-900/40'}`}>
                      <span className="text-base">{CATEGORIES[k].pinSymbol}</span><span className="block text-xs font-extrabold mt-1">{CATEGORIES[k].shortLabel}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <input value={locationName} onChange={e => setLocationName(e.target.value)} placeholder="Локація" className="w-full px-3.5 py-3 rounded-xl bg-slate-900 border border-purple-900/50 text-white text-sm outline-none" />
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <select value={payType} onChange={e => setPayType(e.target.value as PayType)} className="w-full px-3.5 py-3 rounded-xl bg-slate-900 border border-purple-900/50 text-white text-sm outline-none"><option value="fixed">Фіксована</option><option value="hourly">За годину</option><option value="daily">За день</option><option value="monthly">За місяць</option><option value="free">Безкоштовно</option></select>
              </div>
              <div className="grid grid-cols-2 gap-2.5">
                <select value={when} onChange={e => setWhen(e.target.value)} className="w-full px-3.5 py-3 rounded-xl bg-slate-900 border border-purple-900/50 text-white text-sm outline-none"><option>Сьогодні</option><option>Завтра</option><option>Найближчим часом</option><option>Постійно</option></select>
              ...
              <button type="submit" className={`w-full py-3.5 rounded-2xl font-black text-white flex items-center justify-center gap-2 ${isUrgent ? 'bg-rose-600' : 'bg-gradient-to-r from-purple-600 to-indigo-600'}`}><Send className="w-4 h-4" /> Опублікувати оголошення</button>
            </form>
          )}
        </div>
      </div>
