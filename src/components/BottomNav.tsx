...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, setActiveTab, onOpenCreateModal, urgentCount = 0 }) => {
  useEffect(() => {
    if (!window.history.state?.[NAV_STATE]) window.history.replaceState({ ...(window.history.state || {}), [NAV_STATE]: true, tab: activeTab }, '', window.location.href);
    const onPopState = (event: PopStateEvent) => { if (event.state?.[NAV_STATE]) setActiveTab((event.state.tab as ActiveTab) || 'map'); };
